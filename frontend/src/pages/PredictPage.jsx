...
// UNIT HELPER - returns unit string for a given field key
const getUnit = (key) => {
  switch (key) {
...
export default function PredictPage() {
  const [step, setStep] = useState(0); // 0 start, 1..5 steps, 6 = summary-confirm, 7 = result/end
  ...
  const [open, setOpen] = useState(false); // For Building Type dropdown
  ...
  // Load persisted + defaults + ranges + building types
  useEffect(() => {
    const persistedState = JSON.parse(sessionStorage.getItem(STORAGE_KEY));
    if (persistedState) {
    ...
    axios
      .get(`${API_BASE}/defaults/`)
      .then((res) => {
        const def = res.data.defaults || {};
        setDefaults(def);
        setRanges(res.data.feature_ranges || {});
        if (!persistedState || persistedState.step === 0) {
          const initial = { ...def };
          initial.Building_Type = initial.Building_Type || "";
    ...
    axios
      .get(`${API_BASE}/building-types/`)
      .then((res) => setBuildingTypes(res.data.building_types || []))
  ...
  // HANDLE NUMERIC CHANGE (soft allow)
  // Allows intermediate states, backspace, ".", "1.", "-" etc.
  const handleNumericChange = (key, rawValue) => {
    ...
    // Allow intermediate numeric patterns and only those characters
    // Accept digits, optional leading "-", optional single "."
    if (!/^[-]?\d*\.?\d*$/.test(val)) {
      // Ignore invalid characters
      return;
    }

    // Update the form with raw string so user sees what they type
    setForm((prev) => ({ ...prev, [key]: val }));
  };

  // ON BLUR: auto-correct to min/max and format to 2 decimals (Option 2)
  const handleBlurAndFormat = (key) => {
    ...
    // Clamp to min/max if range exists, then format to 2 decimals
    if (ranges && ranges[key]) {
  ...
  // Build payload before sending — clamp again and convert to numbers
  ...
  const handleDownload = () => {
    ...
    const options = {
      margin: [8, 10, 8, 10], // Top, Left, Bottom, Right
  ...
  const confirmAndPredict = async () => {
    ...
    try {
      // Ensure we blur/format all fields before sending - auto-correct
      // Format all known numeric keys to 2 decimals and clamp
      ...
      // Wait a tick for state to update (safe approach)
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        ...
        <div className="w-2/3">
          <input
            ...
            onChange={(e) => {
              // Use numeric handler for numeric fields; building type handled elsewhere
  ...
  // PrintableReport Component (units included in value column)
  const PrintableReport = () => {
    const hasRanges = Object.keys(ranges).length > 0;

    // Helper to format value + unit for PDF
    const pdfValueWithUnit = (k) => {
      ...
      // Otherwise show string value + unit
  ...
  // SummaryCard Component: editable, validate on save, format to 2 decimals
  const SummaryCard = () => {
